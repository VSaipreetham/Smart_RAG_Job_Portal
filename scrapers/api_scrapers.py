...
class RemotiveScraper(BaseScraper):
    def scrape(self):
        ...
        try:
            # Remotive has a generous public API
            url = "https://remotive.com/api/remote-jobs?category=software-dev"
            resp = requests.get(url)
            data = resp.json()
            jobs = data.get('jobs', [])
            
            # Limit to recent 50 to avoid hammering DB on every run
            for j in jobs[:50]:
                title = j.get('title')
                ...
                # Parse date if possible, but keeping it simple
...
class HackerNewsScraper(BaseScraper):
    def scrape(self):
        print("Scraping Hacker News (Who is Hirig)...")
        ...
        try:
            ...
            for kid_id in kids[:30]:
                try:
                    ...
                    c = requests.get(comment_url).json()
